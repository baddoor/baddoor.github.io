...
Write-Host "=== Vercel + GitHub 自动部署脚本启动 ===" -ForegroundColor Cyan
Write-Host "正在监控: '$WatchFolder'" -ForegroundColor Cyan
Write-Host "检测到文件变化后，将自动推送源码到 GitHub 私有仓库" -ForegroundColor Cyan
        ...
        if ($elapsed.TotalSeconds -ge $DebounceSeconds) {
            ...
            try {
                ...
                if ($LASTEXITCODE -ne 0) {
                    ...
                    Write-Host "--> 推送到 GitHub 私有仓库..." -ForegroundColor White
